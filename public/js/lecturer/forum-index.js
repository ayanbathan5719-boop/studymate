// ===== LECTURER FORUM INDEX JAVASCRIPT =====
// This file contains functions for the lecturer forum management page
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.is_pinned) {
                ...
                if (!card.querySelector('.badge-pinned')) {
                    const badge = document.createElement('span');
                    badge.className = 'post-badge badge-pinned';
                    badge.innerHTML = '<i class="fas fa-thumbtack"></i> Pinned';
                    card.insertBefore(badge, card.querySelector('.post-header'));
                }
            } else {
                card.classList.remove('pinned');
                const badge = card.querySelector('.badge-pinned');
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.is_announcement) {
                ...
                // Add announcement badge if not exists
                if (!card.querySelector('.badge-announcement')) {
                    const badge = document.createElement('span');
                    badge.className = 'post-badge badge-announcement';
                    badge.innerHTML = '<i class="fas fa-bullhorn"></i> Announcement';
                    card.insertBefore(badge, card.querySelector('.post-header'));
                }
            } else {
                card.classList.remove('announcement');
                const badge = card.querySelector('.badge-announcement');
                if (badge) badge.remove();
            }
        }
    })
...
// Delete a reply (lecturers can only delete their own replies)
...
// Allows lecturers to edit their own replies within 5 minutes of posting
...
// Note: deletePost() function is now in common.js
// Use it like: deletePost(postId, postUserId)

// Note: submitReplyWithEditor() is defined in the Blade script (TinyMCE integration)
// This file now handles attachment management, nested replies, link previews, and edit replies